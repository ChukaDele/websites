    ...
    <PageShell>
      ...
      >
        ...
        <section id="usual">
          ...
          <p>Every team has its own numbers. Sales quotes one revenue figure, Finance another, and the board pack a third &mdash; each defensible, none agreed. Month-end involves a lot of manual work: someone exports files, pastes them into a spreadsheet, fixes the same errors by hand, and rebuilds the report they rebuilt last month. Ask where a number comes from and the honest answer is often &ldquo;from someone&rsquo;s file&rdquo;, not &ldquo;from the billing system&rdquo;.</p>
          <p>No one is sure what is authoritative &mdash; which source is the one to trust when two disagree. Reports get copied and edited until there are five versions and no original. The person who understands the master spreadsheet becomes a single point of failure; when they are away, reporting stops.</p>
          <p>None of this means anyone did a bad job. It is what happens when a company grows faster than its data setup. Tools were added one at a time, each to solve one problem, and no one stepped back to make them work together. The mess is an accumulation, not a single mistake &mdash; which is also why it will not clear itself up without a deliberate decision about sequence.</p>
        </section>

        <section id="try-first">
          <h2>Start with these checks</h2>
          <p>Before you change anything, write down what you actually have. This is a <strong>data audit</strong> &mdash; a structured review of your sources, definitions and processes &mdash; and you can do the first pass yourself in a day or two. Work through the list below and keep the answers in one document. The point is not to fix as you go; it is to see the whole picture before you decide.</p>
          <TryThisFirst title="Data Diagnostic checklist">
            <ol>
              <li><strong>Business questions.</strong> List the decisions the business must be able to make, and the questions behind them &mdash; &ldquo;which customers are about to leave?&rdquo;, &ldquo;what is our recognised revenue this quarter?&rdquo;. If a question ties to no decision, park it.</li>
              <li><strong>Source inventory.</strong> List every system that holds data that matters &mdash; CRM (the sales system), billing, the product database, the finance tool, and any load-bearing spreadsheets. Note what each one is genuinely the true source for.</li>
              <li><strong>Where reports come from.</strong> For each report leadership relies on, trace where its numbers actually originate. Name the system, the table or the file &mdash; not the person who sends it.</li>
              ...
              <li><strong>Metric definitions.</strong> Check whether your key measures &mdash; &ldquo;active customer&rdquo;, &ldquo;gross margin&rdquo;, &ldquo;churn&rdquo; &mdash; have a single agreed definition, or whether each team quietly uses its own.</li>
              <li><strong>Entity identity.</strong> Check whether the same customer, product or account can be matched reliably across systems. Identity resolution &mdash; linking the records that describe the same real-world thing &mdash; is the hardest part and the one most often skipped.</li>
              ...
              <li><strong>Lineage.</strong> For your most important numbers, can you trace them from the report back to the source row? Lineage &mdash; the documented path a number takes from source to screen &mdash; is what lets you trust a figure rather than hope it is right.</li>
              ...
              <li><strong>Security and access.</strong> Note any constraints &mdash; personal data, regulated data, who is allowed to see what. These shape what is permitted, not just what is possible.</li>
              <li><strong>Build / buy / keep.</strong> Against each area, write one word: <em>build</em> (make something new), <em>buy</em> (use a managed tool), or <em>keep</em> (leave it &mdash; it works). Most areas should end up &ldquo;keep&rdquo;. The audit exists to find the few that should not.</li>
            </ol>
          </TryThisFirst>
        </section>
        ...
        <section id="underneath">
          ...
          <p>Score each finding two ways. <strong>Impact</strong> is how much the problem blocks or distorts a real decision. A wrong revenue definition that misleads the board scores high; a cosmetic label on an internal chart scores low. <strong>Effort</strong> is what the fix will cost you &mdash; time, technical risk, and dependencies on other work or other teams. A fix that needs three systems changed at once is high-effort even when each change is small.</p>
          <p>Plot the findings on an impact &times; effort grid: impact up the side, effort along the bottom. Four regions appear. High impact and low effort are <strong>quick wins</strong> &mdash; do them first. High impact and high effort are <strong>major projects</strong> &mdash; worth doing, but plan and sequence them. Low impact and low effort are <strong>fill-ins</strong> &mdash; do them when you have a spare hour. Low impact and high effort are <strong>thankless</strong> &mdash; leave them.</p>
          <p>Then adjust for sequence, because some fixes make later fixes cheaper. This is where foundational work earns its place. Agreeing definitions and resolving identity are often only medium-impact on their own, but they act as multipliers: once a metric is defined once, and once a customer is one entity everywhere, half of your quality and reconciliation findings become easy. Fixing the dashboard is a surface fix &mdash; it belongs after the definitions it displays are trustworthy, not before. Lineage and ownership are multipliers too: a fix with clear lineage is safe to make, and a fix with a named owner is the only kind that stays fixed.</p>
          <p>One cheap check often earns a place near the top of the list &mdash; how fresh is each source, really? You can answer it in a single query that reads the newest record each source holds:</p>
...
<span className="k">ORDER BY</span> last_updated;   <span className="c">-- oldest source first</span></code></pre>
        ...
        <section id="good">
          ...
          <ul>
            ...
            <li><strong>An agreed sequence.</strong> The findings are ordered, and the order is deliberate &mdash; foundational fixes first, surface fixes after the foundations they depend on. Everyone has seen the sequence, so no one is quietly working on item seven.</li>
          ...
          <table className="article-table">
            ...
            <tbody>
              <tr>
                ...
                <td>Start here &mdash; this is your first fix</td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td>Leave it &mdash; the cost outweighs the value</td>
              </tr>
            </tbody>
          </table>
          <p>The order that falls out of this is nearly always the same: fix the foundations that others depend on, take the quick wins that build confidence, and only then decide whether the major projects &mdash; a warehouse, a permanent team &mdash; are genuinely warranted. Start with the change that makes the next three easier, and the messy setup stops feeling like one impossible problem and starts behaving like a short, ordered list.</p>
        </section>
      </ArticleLayout>
    </PageShell>
  );
